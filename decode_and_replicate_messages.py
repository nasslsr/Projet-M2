...
def decode_insert(byte_data, table_info, table_name,target_db):
    print(byte_data)
    idx = 0
    ...
    insert_query = replicate_insert(table_info, inserted_values, table_name,target_db)
...
def decode_update(byte_data, table_info, table_name,target_db):
    print(byte_data)
    idx = 0
    ...
    update_query = replicate_update(table_info, old_values, new_values, table_name,target_db)
...
def decode_delete(byte_data, table_info, table_name,target_db):
    print(byte_data)
    idx = 1
    ...
    delete_query = replicate_delete(table_info, deleted_values, table_name,target_db)
...
def decode_truncate(byte_data, table_name,target_db):
    print(byte_data)
    idx = 1
    ...
    truncate_query = replicate_truncate(table_name,target_db)
...
def replicate_insert(table_info, inserted_values, table_name,target_db):
    result_insert = {}
    ...
    replicate_queries(insert_template,target_db)
    return insert_template


def replicate_delete(table_info, deleted_values, table_name,target_db):
    result = {}
    ...
    if conditions:
        ...
        replicate_queries(delete_template,target_db)
        return delete_template
    else:
        return "Aucune condition de suppression n'a été spécifiée."


def replicate_update(table_info, old_values, new_values, table_name,target_db):
    result_new = {}
    result_old = {}
    updated_values = []
    ...
    if conditions and updated_values:
        ...
        replicate_queries(update_template,target_db)
        return update_template
    else:
        return "Aucune condition de mise à jour n'a été spécifiée."


def replicate_truncate(table_name,target_db):
    truncate_template = f'TRUNCATE {table_name}'
    replicate_queries(truncate_template,target_db)
    return truncate_template


def decode_message(data, table_info, table_name,target_db):
    ...
    if message_type == 'B':
        ...
        return decode_insert(byte_data, table_info, table_name,target_db)
    elif message_type == 'T':
        return decode_truncate(byte_data, table_name,target_db)
    elif message_type == 'D':
        return decode_delete(byte_data, table_info, table_name,target_db)
    elif message_type == 'U':
        return decode_update(byte_data, table_info, table_name,target_db)
    else:
...
def replicate_queries(query,db_target):
    conn = target_db_connection(db_target)
    cur = conn.cursor()
...
def main(source_db,target_db):
    ...
    with psycopg2.connect(**conn_params) as conn:
        ...
        for lsn, xid, data in changes:
            print(data)
            decode_message(data, table_info, table_name,target_db)
